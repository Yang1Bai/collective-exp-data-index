...
$Transcript = Join-Path `
    $RepoRoot "analysis\results\strength_fatigue_ood_status.log"
$JobSelector = if ($JobId) { "-j '$JobId'" } else { "-u yangbai -n str-fatigue" }
$AccountingSelector = if ($JobId) {
    "-j '$JobId'"
}
else {
    "-S 2026-07-29 -u yangbai --name=str-fatigue"
}
$RemoteCommand = @"
squeue $JobSelector || true
sacct $AccountingSelector --format=JobID,JobName,State,ExitCode,Elapsed,Start,End
echo '--- RESULT FILES ---'
ls -lh '$RemoteRoot/analysis/results/'strength_fatigue_* 2>/dev/null || true
echo '--- STDOUT TAIL ---'
tail -n 50 '$RemoteRoot/logs/'str-fatigue-*.out 2>/dev/null || true
echo '--- STDERR TAIL ---'
tail -n 50 '$RemoteRoot/logs/'str-fatigue-*.err 2>/dev/null || true
"@
