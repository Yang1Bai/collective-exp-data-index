...
$PackageName = "optical_transfer_method_discovery_balam_package.tar.gz"
...
try {
    $RequiredFiles = @(
        ...
        "analysis\prepare_optical_photocatalysis_development_draws.py",
        "analysis\prepare_optical_transfer_method_discovery_draws.py",
        ...
        "analysis\balam\run_optical_transfer_method_discovery_balam.sh",
        "analysis\balam\prepare_and_submit_optical_transfer_method_discovery.sh",
    ...
    & ".\.venv\Scripts\python.exe" "-m" "pytest" `
        "tests\test_optical_transfer_method_discovery.py" "-q"
    if ($LASTEXITCODE -ne 0) {
        throw "Optical method-discovery tests failed."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to create the optical method-discovery package."
    }
    $PackageHash = (
    ...
    $RemoteCommand = "mkdir -p '$RemoteRoot' && mv ~/$PackageName '$RemoteRoot/$PackageName' && cd '$RemoteRoot' && tar -xzf '$PackageName' && bash analysis/balam/prepare_and_submit_optical_transfer_method_discovery.sh"
