...
$RemoteArchive = "/scratch/yangbai/collective-exp-ood/analysis/results/state_matched_mpea_balam_results.tar.gz"
$LocalArchive = Join-Path $env:TEMP "state_matched_mpea_balam_results.tar.gz"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Downloaded state-matched MPEA verification failed."
    }
    Write-Host "State-matched MPEA results downloaded and independently verified."
}
finally {
    Pop-Location
}
