...
$PackageName = "state_matched_mpea_balam_package.tar.gz"
...
try {
    ...
    & ".\.venv\Scripts\python.exe" "-m" "json.tool" `
        "analysis\state_matched_mpea_balam_design.json" | Out-Null
    ...
    & tar.exe -czf $PackagePath `
        ...
        "analysis\balam\run_state_matched_mpea_balam.sh" `
        "analysis\balam\prepare_and_submit_state_matched_mpea.sh" `
    ...
    $RemoteCommand = "mkdir -p '$RemoteRoot' && mv ~/$PackageName '$RemoteRoot/$PackageName' && cd '$RemoteRoot' && tar -xzf '$PackageName' && bash analysis/balam/prepare_and_submit_state_matched_mpea.sh"
